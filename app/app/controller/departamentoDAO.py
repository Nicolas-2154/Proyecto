...
def crearDepartamento(departamento):
    try:
        ...
        if not cone:
            return
        cursor = cone.cursor()
        sql = "INSERT INTO departamento(idDepartamento, nombre, idGerente) VALUES(%s,%s,%s)"
        cursor.execute(sql, (departamento.getID(), departamento.getNombre(), departamento.getIdGerente()))
        cone.commit()
        return True
