...
def _formatear_proyectos(datos):
    if not datos:
        print("No hay proyectos registrados.")
        return

    encabezados = ["ID", "Nombre", "Descripción", "Fecha inicio"]
    anchos = [8, 24, 38, 14]
    ...
    for fila in datos:
        idProyecto, nombre, descripcion, fecha_inicio = fila[:4]
        columnas = [
            str(idProyecto).center(anchos[0]),
            str(nombre)[: anchos[1]].ljust(anchos[1]),
            str(descripcion)[: anchos[2]].ljust(anchos[2]),
            str(fecha_inicio).center(anchos[3]),
        ]
